							...
							<form action="insertRow.php" method="post">
								...
								<ul class="field-group">
									<?php for ($i = 1; $i < count($columns); $i++): ?>
										<li>
											<label
												for="<?php echo $columns[$i] ?>"><?php echo strtoupper($columns[$i]) ?>
												:</label>
											<input type="text" id="<?php echo $columns[$i] ?>"
											       name="<?php echo $columns[$i] ?>">
										</li>
									<?php endfor; ?>
								</ul>
				...
				<div class="col-md-12">
					<div class="table-responsive">
						<table width="100%" class="table table-striped">
							<tr>
								<td colspan="5">
									<table id="recordData" cellpadding="0" cellspacing="0" border="0" colspan="5"
									       width="100%">
										<thead>
										<tr id="rowHeader">
											<?php for ($i = 1; $i < count($columns); $i++): ?>
												...
												<td class="deleteFixedWidthCell">
													<form method="post" action="deleteRow.php">
														...
														<button class="buttonSecondary" type="submit"
														        onclick="return confirm('Confirm to delete?');">Delete
															Record
														</button>
													</form>
												</td>
